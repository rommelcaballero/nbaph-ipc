...
<head>
<title>NBA Philippines | Event Albums</title>
<?php
include('head.php');
?>
...
<?php
$results = $connect->query("select * from eventsalbum where AlbumID = '" . $connect->real_escape_string($_GET['album']) . "'");
$row = $results->fetch_array();
$album = stripslashes($row['AlbumName']);
$ex = explode("-", $row['DateAdded']);
$date = date("M j, Y", mktime(0, 0, 0, $ex[1], $ex[2], $ex[0]));

$results = $connect->query("select * from eventsphotos where AlbumID = '" . $connect->real_escape_string($_GET['album']) . "'");
