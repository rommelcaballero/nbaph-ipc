...
if ($sblog_id) {
   $query = "SELECT * FROM blog WHERE BlogID='".mysqli_real_escape_string($connect, $sblog_id)."'";
}
else {
   $query = "SELECT * FROM blog WHERE Blogger='".mysqli_real_escape_string($connect, $blogger)."' ORDER BY DatePosted DESC LIMIT 0, 4";
}
...
if($found <= 0) {
   ...
   if ($sblog_id)
      $cachefile = 'cached/blogs_full-' . $sblog_id . '-cache-'.$last_file.'.html';
   else
      $cachefile = 'cached/blogs_full-' . $blogger . '-cache-'.$last_file.'.html';
}
...
if(!file_exists($cachefile)) {
   ...
   while($row = mysqli_fetch_array($result)) {
      ...
      $blog_array[$count]['BlogBody'] = ucfirst(trim(($row['BlogBody'])));
   ...
   $query_more = "SELECT * FROM blog WHERE Blogger='".mysqli_real_escape_string($connect, $blog_array[0]['Blogger'])."' and DisplayOn <= NOW() ORDER BY DatePosted DESC LIMIT 0, 20";
