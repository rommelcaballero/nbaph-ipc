...
if ($_GET['writer'] == "") {
   header("Location: news-writers.php");
   exit(0);
}
?>
...
<?php
$results = $connect->query("select * from writers_stories where Blogger = '" . $connect->real_escape_string(urldecode($_GET['writer'])) . "' order by DisplayOn desc limit 0, 8");
         ...
         <div class="nbaph_news-writers_cell" style="clear: both; width: 100%">
            <div class="nbaph_news-writers_padding">
               <div class="nbaph_news-writers_title">
                  <a href="news-writers_article.php?id=<?php echo $row['BlogID']; ?>" style="font-size: 18px"><?php echo stripslashes($row['BlogTitle']); ?></a>
               </div>
               ...
               <div class="nbaph_news-writers_links">
                  <a href="news-writers_article.php?id=<?php echo $row['BlogID']; ?>" style="font-size: 15px">Full Story</a>
               </div>
            </div>
         </div>
<?php
   ...
   <div class="nbaph_article_standard">
      ...
      <div id="nbaph_writers_more" style="margin-top: 10px">
         <div class="nbaph_recent_content">
            MORE FROM <?php echo strtoupper(urldecode($_GET['writer'])); ?>
         </div>

<?php
$results = $connect->query("select * from writers_stories where Blogger = '" . $connect->real_escape_string(urldecode($_GET['writer'])) . "' order by DisplayOn desc limit 8, 100");

while($row = $results->fetch_array()) {
?>
         <div class="nbaph_writers_more_entry">
            <a href="news-writers_article.php?id=<?php echo $row['BlogID']; ?>"><?php echo stripslashes($row['BlogTitle']); ?></a>
            <?php
