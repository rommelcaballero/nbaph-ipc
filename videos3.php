...
<head>
	...
	if(isset($_SERVER['HTTP_X_FORWARDED_FOR'])) {
		...
		if($has_many_ip){
			$exploded_ip = explode(',',$x_forwarded);
			$remote_addr = $exploded_ip[0];
		}else{
	...
	Tag:  <input type="text" name="testtag" size="70px">
	<input type="submit" name="tagsubmit" value="Update Tag">
	</form>
	</center>
		<p>&nbsp;</p>
		<div id="fb-root"></div>						
		...
		$(document).ready(function(){
			...
			function requestAds() {
				...
				adsRequest = {
					adTagUrl : "http://ad4.liverail.com/?LR_PUBLISHER_ID=45357&LR_SCHEMA=vast2-vpaid&LR_URL=[URL],&LR_VERTICAL=Business&LR_VIDEO_AMID=ROS&LR_CONTENT=6&LR_AUTOPLAY=0&LR_TITLE=[LR_TITLE]&LR_VIDEO_ID=[BLR_VIDEO_ID]&CACHEBUSTER=[CACHEBUSTER]"
					};	
